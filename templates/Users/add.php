    ...
    <div class="col-md-6">
      <div class="card">
        ...
        <div class="card-body">
          ...
          <div class="form-group">
            <?= $this->Form->control('role', ['required' => true, 'class' => 'form-control', 'id' => 'role', 'placeholder' => 'role','options' => ['admin' => 'Admin', 'author' => 'Author']]) ?>
          </div>
          <?= $this->Form->button(__('Submit'), ['class' => 'btn btn-primary btn-block', 'onclick' => 'return validateForm(event);']) ?>
          <?= $this->Form->end() ?>
        </div>
      </div>
    </div>
...
<script>
    ...
    if (password.indexOf("@") == -1) {
        alert("Password must contain the special character '@' ");
        if(event.type === "submit") {
        event.preventDefault(); // Prevent the form from submitting
        }
        return false;
